...
namespace GitCommands.UserRepositoryHistory
{
    ...
    public sealed class LocalRepositoryManager // : IRepositoryManager
    {
        ...
        public IList<Repository> AddAsMostRecent(string repositoryPath)
        {
            ...
            return AddAsMostRecentRepository(repositoryPath);
        }

        IList<Repository> AddAsMostRecentRepository(string path)
        {
                // await TaskScheduler.Default;
                var repositoryHistory = LoadHistory();
        ...
        public IList<Repository> LoadHistory() // not Async()
        {
            // await TaskScheduler.Default;
        ...
        /// <exception cref="ArgumentException"><paramref name="repositoryPath"/> is <see langword="null"/> or <see cref="string.Empty"/>.</exception>
        [ContractAnnotation("repositoryPath:null=>halt")]
        public IList<Repository> RemoveFromHistory(string repositoryPath)
        {
            if (string.IsNullOrWhiteSpace(repositoryPath))
            {
                throw new ArgumentException(nameof(repositoryPath));
        ...
        public void SaveHistory(IEnumerable<Repository> repositoryHistory)
        {
            // await TaskScheduler.Default;
