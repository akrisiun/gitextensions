...
{
    public partial class FormPullX : GitModuleForm, IWin32Window2
    {
        #region Translation
        ...
        private FormPullX() : this(null, null, null)
        { }

        public FormPullX(GitUICommands aCommands, string defaultRemoteBranch, string defaultRemote)
            : base(aCommands)
        {
            InitializeComponent();
            Translate();
        ...
        public DialogResult PullChanges(IWin32Window owner)
        {
            ...
            if (!Fetch.Checked && Branches.Text.IsNullOrWhiteSpace() && Module.IsDetachedHead())
            {
                ...
                {
                    case 0:
                        if (!UICommands.StartCheckoutBranch(owner, null, false, new[] {""}))
